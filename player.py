import pygame, os, math
from animation import load_animations
key_binding = "Normal"

def flip(img, boolean):
    pygame.transform.flip(img, boolean, False)

def collision_detection(rect1, rect_list):
    hit_list = []
    for rect in rect_list:
        if rect.colliderect(rect1):
            hit_list.append(rect1)
    return hit_list

class Player(pygame.sprite.Sprite):
    def __init__(self, loc):
        super().__init__()
        # player animation
        self.import_animation()
        self.frame = 0
        self.action = ''
        self.change_action('idle')
        self.flip = False
        self.collide_bottom = False

        self.image = self.animations[self.action][self.anim[self.frame]]
        self.rect = self.image.get_rect(topleft = loc)



        #player movement
        self.x = loc[0]
        self.y = loc[1]
        self.movement = [0, 0]
        self.air_timer = 0
        self.velocity = 0
        self.vertical_momentum = 0
        self.direction = [False, False]
        self.jump_held = False
        self.gravity_multiplier = 0.2
        self.n = 0

    def get_input(self):

        self.movement = [0, 0]
        keys = pygame.key.get_pressed()
        if keys[pygame.K_RIGHT] and key_binding == "Normal":
            self.movement[0] += 3
            self.direction = [True, False]
            self.flip = False
            if -0.2 < self.vertical_momentum < 0.2 and self.jump_held:
                self.movement[0] += 1
        elif keys[pygame.K_d] and key_binding == "WASD":
            self.movement[0] += 3
            self.direction = [True, False]
            self.flip = False
            if -0.2 < self.vertical_momentum < 0.2 and self.jump_held:
                self.movement[0] += 1
        elif keys[pygame.K_LEFT] and key_binding == "Normal":
            self.movement[0] -= 3
            self.direction = [False, True]
            self.flip = True
            if -0.2 < self.vertical_momentum < 0.2 and self.jump_held:
                self.movement[0] -= 1
        elif keys[pygame.K_a] and key_binding == "WASD":
            self.movement[0] -= 3
            self.direction = [False, True]
            self.flip = True
            if -0.2 < self.vertical_momentum < 0.2 and self.jump_held:
                self.movement[0] -= 1
        else:
            self.direction = [False, False]
        if keys[pygame.K_SPACE] and key_binding == "Normal":
            if self.air_timer < 6:
                self.vertical_momentum = -3
        elif keys[pygame.K_w] and key_binding == "WASD":
            if self.air_timer < 6:
                self.vertical_momentum = -3

    def gravity(self):
        self.gravity_multiplier = 0.2
        if self.vertical_momentum > 0.4:
            self.gravity_multiplier = 0.5
        elif self.vertical_momentum < 0 and not(self.jump_held):
            self.vertical_momentum += 0.5
        elif -0.2 < self.vertical_momentum < 0.2 and self.jump_held:
            self.gravity_multiplier = 0.05
            self.velocity = 5
        self.movement[1] += self.vertical_momentum
        self.vertical_momentum += self.gravity_multiplier
        if self.vertical_momentum > 3:
            self.vertical_momentum = 3

    def import_animation(self):
        path = 'data/graphics/'
        self.animations = {'idle': [], 'run': [], 'jump':[], 'fall': []}
        animation_data = load_animations(path)
        self.animation_frames = animation_data[0]
        for animation in self.animations.keys():
            self.animations[animation] = animation_data[1]

    def flip(self, boolean):
        self.flip = boolean

    def change_action(self, new_action):
        if self.action == new_action:
            pass
        else:
            self.action = new_action
            self.frame = 0
            self.anim = self.animation_frames[self.action]


    def implement_anim(self, loop):
        self.frame += 1
        while self.frame >= len(self.anim):
            if loop:
                self.frame -= len(self.anim)
        image = self.animations[self.action][self.anim[self.frame]]
        self.image = pygame.transform.flip(image, self.flip, False)

    def status(self):
        if self.movement[1] < 0:
            self.change_action('jump')
        elif self.movement[1] > 1:
            self.change_action('fall')
        else:
            if self.movement[0] == 0:
                self.change_action('idle')
            if self.movement[0] > 0:
                self.change_action('run')
            if self.movement[0] < 0:
                self.change_action('run')

    def update(self, scroll):
        self.get_input()
        # gravity
        self.gravity()
        self.movement[1] += self.vertical_momentum


        self.rect.x -= scroll[0]
        self.rect.y -= scroll[1]

        self.implement_anim(True)
        self.status()













